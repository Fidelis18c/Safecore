export default function SectionHeader({ badge, title, subtitle, align = 'center', light = false, plainBadge = false, bigBadge = false }) {
      ...
      {badge && (
        plainBadge ? (
          <span className={`inline-block font-bold ${
            bigBadge
              ? 'text-3xl md:text-4xl lg:text-[2.75rem] font-extrabold leading-tight mb-4'
              : 'text-xs tracking-wider uppercase mb-4'
          } ${
            light ? 'text-white/90' : bigBadge ? 'text-brand-navy' : 'text-brand-green'
          }`}>
            {badge}
          </span>
        ) : (
